...
private:
    ...
    UUID                ListenToNewSocket   (const char *address, u_short port, UUID id);           // Start listening to new read/write event on this socket

public:
                        SocketClient        ();
                        ~SocketClient       ();
    inline UUID         ListenToNewSocket   (const char *address, u_short port)                     { UUID nullId; UuidCreateNil(&nullId); return ListenToNewSocket(address, port, nullId); }
